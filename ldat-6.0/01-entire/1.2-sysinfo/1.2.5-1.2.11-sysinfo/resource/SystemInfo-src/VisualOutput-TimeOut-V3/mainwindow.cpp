    ...
    initializeFlag(true)
{
    ...
    if (argc >= 2) {
        ...
        printf("Please input output directory path\n");
        return;
//        QString currentPath = QCoreApplication::applicationDirPath();
//        int index = currentPath.indexOf("resource"); // current file is under resource dir
//        outputDirPath = currentPath.mid(0, index) + tr("output");
    }
//    printf("Outside dir %s\n", outputDirPath.toStdString().c_str());
...
//    QString currentPath = QCoreApplication::applicationDirPath();
    ...
    std::string currentPathStr = outputDirPath.toUtf8().constData();
    std::string fileTotoalPathStr = fileTotalPath.toUtf8().constData();
    printf("currentPath %s, totalPath %s\n", currentPathStr.c_str(), fileTotoalPathStr.c_str());

    QFile recordFile(fileTotalPath);
    if (!recordFile.open(QIODevice::WriteOnly)) {
        printf("record file open error, %s\n", fileTotoalPathStr.c_str());
        return;
    }
...
//        for (int i = 0; i < maxEthNumber; ++ i) {
//            QString ethNumber;
//            ethNumber.setNum(i);
//            QString ethName = "eth" + ethNumber;

//            if (testOuput.contains(ethName)) {
//                QString actualCommand = "ethtool " + ethName;
//                totalOutput += outputCommand(actualCommand) + "\n";
//            }
//        }
    }
...
QString MainWindow::outputCommand(QString command)
{
    ...
    // Qt 4
    std::string tempCmd = command.toUtf8().constData();
    fp = popen(tempCmd.c_str(), "r");
...
void MainWindow::FillNewTab(QTabWidget *parentTabWidget, QTextBrowser *childTextBrowser, QString barTitle, QString content)
{
    ...
    if (initializeFlag) { // add content at the first time to fill tabwidgets
...
QString MainWindow::OutputSpecialCommand(QString specialCommand)
{
    QString totalOutput = "";
    if (specialCommand.contains("ethtool")) {
//        QString testOuput = outputCommand("ifconfig");
        QString testOuput = outputCommand("ifconfig | awk -F \"[: ]\" \'BEGIN{RS = \'\\n\'} {if (($0 ~ /.*BROADCAST.*/) && ($0 ~ /.*MULTICAST.*/) && ($0 ~/.*UP.*/)&& ($0 !~/.*LOOPBACK.*/)) {print $1}}\'");
        QString ethName = testOuput.simplified();
