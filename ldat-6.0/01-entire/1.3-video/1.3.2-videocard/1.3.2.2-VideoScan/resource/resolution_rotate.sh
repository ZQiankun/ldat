	...
	if [ ${i} -lt $(expr ${NUM_OUTPUTS} - 1) ]; then
		...
		for MODE in ${MODES[@]}; do
			...
			for RATE in ${RATES[@]}; do
				...
				echo -n [$(date "+%y-%m-%d %T")]: >> ${LOG_FILE}
				echo " xrandr --output ${OUTPUTS[${i}]} --rotate left" >> ${LOG_FILE}
				xrandr --output ${OUTPUTS[${i}]} --rotate left
				sleep 8

				echo -n [$(date "+%y-%m-%d %T")]: >> ${LOG_FILE}
				echo " xrandr --output ${OUTPUTS[${i}]} --rotate inverted" >> ${LOG_FILE}
				xrandr --output ${OUTPUTS[${i}]} --rotate inverted
				sleep 8

				echo -n [$(date "+%y-%m-%d %T")]: >> ${LOG_FILE}
				echo " xrandr --output ${OUTPUTS[${i}]} --rotate right" >> ${LOG_FILE}
				xrandr --output ${OUTPUTS[${i}]} --rotate right
				sleep 8

				echo -n [$(date "+%y-%m-%d %T")]: >> ${LOG_FILE}
				echo " xrandr --output ${OUTPUTS[${i}]} --rotate normal" >> ${LOG_FILE}
				xrandr --output ${OUTPUTS[${i}]} --rotate normal
				sleep 8
